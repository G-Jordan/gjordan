...
(function () {
  window.addEventListener("DOMContentLoaded", () => {
    ...
    const saved = window.G73ThemeBridge?.loadTheme?.() || (() => { const savedRaw = localStorage.getItem("themeSettings"); return savedRaw ? safeParse(savedRaw) : null; })();
    ...
    if (!saved) {
      if (window.G73ThemeBridge?.saveTheme) window.G73ThemeBridge.saveTheme(theme);
      else localStorage.setItem("themeSettings", JSON.stringify(theme));
    }
  });
})();